import numpy as np
from CIC.get_bounded_max import get_bounded_max
from CIC.util import length, nchoosek
from CIC.get_max import get_max
        ...
        if(length(sym_bnd) == 0):
            ...
            if(length(fin_sym) == 0):
                if length(pot_sym_cic) == 0 and length(pot_sym) != 0:
                    ...
                    sdev = np.std(intf_wind[:,(n_pnt * pot_sym_cic).astype('int')-1])
                    b = np.argmin(sdev)
