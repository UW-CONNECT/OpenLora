...
from NScale.classes import CWin, CSymbol
...
def detect(winset, num_preamble, verbose=False):
    ...
    for i in range(0, len(winset)):
        ...
        for sym in symbset:
            ...
            if any(np.array(state_keys) - (sym.fft_bin) >= 2**SF - 2):
                I, key = pf.nearest(np.mod(np.array(state_keys)+2, 2**SF), np.mod(sym.fft_bin, 2**SF+1), 2)
                key = state_keys[I]
            elif any(sym.fft_bin - np.array(state_keys) >= 2**SF - 2):
                I, key = pf.nearest(np.array(state_keys), np.mod(sym.fft_bin+ 2, 2**SF+1), 2)
                key = state_keys[I]
            if I < 0:
            ...
            I, key = pf.nearest(np.array(state_keys), np.mod(sym.fft_bin + config.SW1, 2**SF+1), 2)
            if any(np.array(state_keys)-(sym.fft_bin+config.SW1) >= 2**SF - 2):
                I, key = pf.nearest(np.mod(np.array(state_keys)+2, 2**SF), np.mod(sym.fft_bin + config.SW1, 2**SF+1), 2)
                key = state_keys[I]
            elif any((sym.fft_bin+config.SW1) - np.array(state_keys) >= 2**SF - 2):
                I, key = pf.nearest(np.array(state_keys), np.mod(sym.fft_bin + config.SW1 + 2, 2**SF+1), 2)
                key = state_keys[I]
            ...
            I, key = pf.nearest(np.array(state_keys), np.mod(sym.fft_bin + config.SW2, 2**SF+1), 2)
            if any(np.array(state_keys)-(sym.fft_bin+config.SW2) >= 2**SF - 2):
                I, key = pf.nearest(np.mod(np.array(state_keys)+2, 2**SF), np.mod(sym.fft_bin + config.SW2, 2**SF+1), 2)
                key = state_keys[I]
            elif any((sym.fft_bin+config.SW2) - np.array(state_keys) >= 2**SF - 2):
                I, key = pf.nearest(np.array(state_keys), np.mod(sym.fft_bin + config.SW2 + 2, 2**SF+1), 2)
                key = state_keys[I]
